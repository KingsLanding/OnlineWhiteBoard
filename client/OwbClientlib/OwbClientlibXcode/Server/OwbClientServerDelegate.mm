...
- (BOOL)setDocument:(NSString*) meetingCode_ WithSerialNumber:(int)docNumber {
    std::string mi = std::string([meetingCode_ UTF8String]);
    return SERVER->SetDocument(mi, docNumber);
}
...
{
    std::string mi = std::string([meetingId UTF8String]);
    NSLog(@"resume Updater .... meetingCode%@",meetingId);
