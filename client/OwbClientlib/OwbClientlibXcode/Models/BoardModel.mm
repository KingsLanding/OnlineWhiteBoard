...
- (void) drawWhite:(CGContextRef) canvas Rect:(CGRect) rect;
@end
...
- (id) init
{
    ...
    if (nil != self) {
        ...
        readOpQueue = dispatch_queue_create(READQUEUE, NULL);
        ...
        for (int i=0; i<BOARD_NUM; i++) {
            ...
            [self drawWhite:context_[i] Rect:CGRectMake(0, 0, BW(i), BH(i))];
...
- (void) drawWhite:(CGContextRef) canvas Rect:(CGRect) rect
{
    CGContextSetAlpha(canvas, 1.0);
    CGColorRef color_ref = Kingslanding::OnlineWhiteBoard::Client::Tools::CGEraserColor();
    CGContextSetFillColorWithColor(canvas, color_ref);
...
//            [self drawWhite:context_[i] Rect:CGRectMake(0, 0, BW(i), BH(i))];
    ...
    dispatch_sync(dispatch_get_main_queue(), ^(void){[displayer_ display];});
}
...
{
    dispatch_async(readOpQueue, ^(void){[self readOperationQueue];});
}
...
//            dispatch_sync(dispatch_get_main_queue(), ^(void){[displayer_ display];});
        ...
        dispatch_async(dispatch_get_main_queue(), ^(void){[displayer_ display];});*/
//        dispatch_async(dispatch_get_main_queue(), ^(void){[displayer_ drawOp:op];});
        [self drawOperationBack:op];
        [op release];
        dispatch_async(dispatch_get_main_queue(), ^(void){[displayer_ display];});
    }
...
- (void)trigerDrawSelf
{
    dispatch_async(boardOpQueue, ^(void){[self drawSelf];});
}
